"""ISO 3166-1 alpha-2 → alpha-3 conversion, plus known non-country aggregate codes.

`TimeSeriesRecord.country_code` is ISO-3 everywhere in EconRadar, but BIS reports its
`REF_AREA` dimension in ISO-2, so its rows need converting before they can be stored.

The map is the full ISO 3166-1 set rather than only the codes BIS happens to publish
today. A partial map would turn a newly-added reference area into a silent gap; with
the full set, an unknown code is a genuine anomaly and the caller raises so the record
reaches `etl_errors` visibly.
"""
...
#: Codes that are legitimately not countries. These are skipped, not errors: BIS
#: publishes a euro-area aggregate alongside its member states by design.
AGGREGATE_ALPHA2: Final[frozenset[str]] = frozenset({"XM", "EU", "XK", "ZZ"})

ALPHA2_TO_ALPHA3: Final[dict[str, str]] = {
...
class UnknownCountryCode(ValueError):
    """A country code could not be resolved to ISO-3."""


def is_aggregate(alpha2: str) -> bool:
    return alpha2.strip().upper() in AGGREGATE_ALPHA2


def to_alpha3(alpha2: str) -> str:
    """Convert ISO-2 to ISO-3. Raises UnknownCountryCode for anything unmapped."""
    code = alpha2.strip().upper()
    try:
        return ALPHA2_TO_ALPHA3[code]
    except KeyError:
        raise UnknownCountryCode(f"unmapped ISO 3166-1 alpha-2 code: {code!r}") from None
